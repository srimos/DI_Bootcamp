...
select * from film order by rental_rate asc, title asc limit 10;

-- 11. Not satisfied with the results. Write a query which will find the next 10 cheapest movies.
select * from film order by rental_rate asc, title asc limit 10 offset 10;
-- Bonus: Try to not use LIMIT.
select * from film order by rental_rate asc, title asc offset 10 fetch next 10 rows only;
...
select distinct film.title, count(inventory.film_id) as inventory_status from film left outer join inventory on film.film_id=inventory.film_id group by film.title, inventory.film_id having inventory.film_id is null;
